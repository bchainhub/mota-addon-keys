...
<script lang="ts">
	...
	async function loadRegistry() {
		...
		const [signRes, oricRes, finRes] = await Promise.all(paths.map((p) => fetch(p)));
		...
		if (signRes.ok) {
			try {
				...
				if (isSigningDoc(raw)) {
					...
					w.push('signing-keys.json: expected an object with keys[] (each entry needs kid)');
				}
			} catch {
				w.push('signing-keys.json: invalid JSON');
			}
		}
	...
	{:else if doc !== null}
		...
		{#if !hasAnySection}
			<p class="text-gray-600 dark:text-gray-400 mb-4">
				...
				<code class="text-xs">signing-keys.json</code>,
		...
		{#if doc.signing && doc.signing.keys.length > 0}
			<section id="keys-signing" class="mb-10 scroll-mt-20">
				...
				<p class="mb-2 text-xs text-gray-500 dark:text-gray-500">
					Source: <code>/.well-known/signing-keys.json</code>
				</p>
